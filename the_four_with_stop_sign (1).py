...
def main():
    # Initialize camera for visual object detection display
    print("Starting camera and object detection display...")
    ...
    try:
        while True:
            # Stop sign interrupt check - completely separate from navigation
            if time.time() - last_stop_time > 5:  # 5 second cooldown
                sign_type = Vilib.traffic_sign_obj_parameter.get('t', 'none')
                if sign_type == 'stop':
                    acc = Vilib.traffic_sign_obj_parameter.get('acc', 0)
                    if acc > 60:  # 60% confidence threshold
                        ...
                        # Continue with normal loop after interrupt
