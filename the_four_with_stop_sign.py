...
STOP_SIGN_WAIT_SEC = 3.0  # How long to wait at stop sign
...
def check_for_stop_sign():
    """Check if a stop sign is detected by the camera"""
    sign_type = Vilib.traffic_sign_obj_parameter['t']
    if sign_type == 'stop':
        acc = Vilib.traffic_sign_obj_parameter['acc']
        # Only react if confidence is high enough
        if acc > 60:  # 60% confidence threshold
            return True
    return False

def handle_stop_sign(px: Picarx):
    """Stop the robot when a stop sign is detected"""
    print("STOP SIGN DETECTED! Stopping for 3 seconds...")
    px.stop()
    px.set_dir_servo_angle(0)
    
    # Visual/audio feedback could be added here
    # e.g., flash LEDs, beep, etc.
    
    time.sleep(STOP_SIGN_WAIT_SEC)
...
def main():
    ...
    print("Starting camera and traffic sign detection...")
    Vilib.camera_start(vflip=False, hflip=False)
    Vilib.show_fps()
    Vilib.display(local=True, web=True)  # Shows visual feed
    Vilib.traffic_detect_switch(True)    # Enable traffic sign detection
    ...
    print("Enter destination x y (x in [-2..2], y in [0..5]) relative to start (0,0):")
    while True:
        ...
        if len(parts) == 2:
            try:
                ...
                pass
        print("Invalid. Try again (e.g., 1 3):")

    cur = (0, 0)
    heading = "North"
    consecutive_blocked = 0
    last_escape_direction = None
    last_stop_time = 0  # Track when we last stopped for a sign

    try:
        while True:
            # Check for stop sign periodically (but not too often after just stopping)
            current_time = time.time()
            if current_time - last_stop_time > 5.0:  # Don't check for 5 seconds after stopping
                if check_for_stop_sign():
                    handle_stop_sign(px)
        ...
        px.stop()
        Vilib.camera_close()
        print("Camera closed, navigation stopped.")
