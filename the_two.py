...
class HW:
    def __init__(self):
        ...
        try:
            ...
            print(f"[WARN] Picarx init failed: {e}\nRunning in SIM mode.", file=sys.stderr)

    def steer(self, angle):
        if self.px:
            try: self.px.set_dir_servo_angle(angle)
            except Exception as e: print(f"[WARN] steer: {e}", file=sys.stderr)
        else:
            print(f"[SIM] steer {angle}°")

    def scan_servo(self, angle):
        # Use steering servo to sweep if no dedicated pan
        self.steer(angle)

    def fwd(self, speed):
        if self.px:
            try: self.px.forward(speed)
            except Exception as e: print(f"[WARN] forward: {e}", file=sys.stderr)
        else:
            print(f"[SIM] forward speed={speed}")

    def back(self, speed):
        if self.px:
            try: self.px.backward(speed)
            except Exception as e: print(f"[WARN] backward: {e}", file=sys.stderr)
        else:
    ...
    def stop(self):
        if self.px:
            try: self.px.stop()
            except Exception as e: print(f"[WARN] stop: {e}", file=sys.stderr)
        else:
            print("[SIM] stop")
    ...
    def dist_cm(self):
        if self.px:
            try:
                d = self.px.get_distance()
                return d if d is not None else MAX_RANGE_CM
            except Exception:
                pass
        return MAX_RANGE_CM  # SIM: clear
...
# ----------------- Scan and integrate (barebones) -----------------
def sweep_and_update(hw: HW, grid: Grid):
    pts = []
    for ang in range(SCAN_MIN_DEG, SCAN_MAX_DEG + 1, SCAN_STEP_DEG):
        hw.scan_servo(ang)
        time.sleep(0.01)
        d = hw.dist_cm()
        d = max(0, min(MAX_RANGE_CM, d if d is not None else MAX_RANGE_CM))
        pts.append((ang, d))
    hw.scan_servo(0)
...
class Move:
    ...
    def forward_one(self):
        self.hw.steer(0)
        self.hw.fwd(SPEED_FORWARD)
        time.sleep(T_FORWARD_CELL_SEC)
        self.hw.stop()

    def backward_one(self):
        self.hw.steer(0)
        self.hw.back(SPEED_BACKWARD)
        time.sleep(T_BACKWARD_CELL_SEC)
        self.hw.stop()

    def three_point_left(self):
        t1,t2,t3 = TURN_LEFT_SEQ
        self.hw.steer(+STEER_MAX_DEG); self.hw.fwd(SPEED_FORWARD); time.sleep(t1); self.hw.stop()
        self.hw.steer(-STEER_MAX_DEG); self.hw.back(SPEED_BACKWARD); time.sleep(t2); self.hw.stop()
        self.hw.steer(+STEER_MAX_DEG); self.hw.fwd(SPEED_FORWARD); time.sleep(t3); self.hw.stop()
        self.hw.steer(0)
    ...
    def step_toward(self, cur, nxt):
        dx, dy = nxt[0]-cur[0], nxt[1]-cur[1]
        if (dx, dy) == (0, 1):   # forward
            self.forward_one(); return "FWD"
        if (dx, dy) == (-1, 0):  # left turn + forward
            self.three_point_left(); self.forward_one(); return "L+F"
        if (dx, dy) == (1, 0):   # right turn + forward
            self.three_point_right(); self.forward_one(); return "R+F"
        if (dx, dy) == (0, -1):  # backward 1
            self.backward_one(); return "BACK"
        return "NONE"
...
def main():
    ...
    while True:
        ...
        # Ensure at least one step exists beyond current
        if len(path) < 2:
            # Path only contains current; scan to discover more
            sweep_and_update(hw, g)
            continue
        ...
        # Execute one step
        nxt = path[1]
        tag = mover.step_toward(cur, nxt)
        print("Move:", tag, "to", nxt)
        cur = nxt  # ego frame: we keep coordinates relative to start per spec
